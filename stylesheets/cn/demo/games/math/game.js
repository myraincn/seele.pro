...
window.onload = function() {
    ...
    getUrl = "https://www.seele.pro/draw/get/";
    function getScore() {
         fetch(getUrl)
...
titleScreen.prototype = {
    create: function(){
        ...
        game.add.bitmapText(game.width / 2, game.height - 200, "font", "SCORE ON CHAIN", 48).anchor.set(0.5);
        //game.add.bitmapText(game.width / 2, game.height - 100, "bignumbersfont", savedData.score.toString(), 90).anchor.set(0.5);
        game.add.bitmapText(game.width / 2, game.height - 100, "font", score_on_chain, 70).anchor.set(0.5);
    },
    startGame: function(){
...
theGame.prototype = {
	...
	createLevel: function(){
        ...
        this.infoText = game.add.text(game.width / 2, item.y, "任意方向连接数字以求和获得分数", 18)
        ...
        if(!this.gameOver){
            for(i = 0; i < 5; i++){
                if(totalSum == this.targetsArray[i].numberToMatch){
                    ...
                    this.targetsArray[i].energy = 360; //original 360
...
gameOver.prototype = {
    create: function(){
        ...
        game.add.text(game.width / 2, 100, "分数", 48).anchor.set(0.5);
        ...
        game.add.bitmapText(game.width / 2, game.height - 450, "font", "REPLAY", 48).anchor.set(0.5);
        //game.add.bitmapText(game.width / 2, game.height - 100, "font", bestScore, 90).anchor.set(0.5);
    ...
    startGame: function(){
        postUrl = "https://www.seele.pro/draw/post/?score=" + score;
        console.log(postUrl);
        function postScore() {
             fetch(postUrl)
             .then(function(response){
